defmodule Provider.Scim.Lifecycle do
  ...
  def after_create(resource_type, scim_response, caller) do
    Logger.info("[SCIM Lifecycle] #{caller.id} created #{resource_type} #{scim_response["id"]}")
  ...
  def after_replace(resource_type, scim_response, caller) do
    Logger.info("[SCIM Lifecycle] #{caller.id} replaced #{resource_type} #{scim_response["id"]}")
  ...
  def after_patch(resource_type, scim_response, caller) do
    Logger.info("[SCIM Lifecycle] #{caller.id} patched #{resource_type} #{scim_response["id"]}")
  ...
  def after_delete(resource_type, resource_id, caller) do
    Logger.info("[SCIM Lifecycle] #{caller.id} deleted #{resource_type} #{resource_id}")
